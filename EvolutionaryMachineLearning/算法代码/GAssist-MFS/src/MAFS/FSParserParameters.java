...
public class FSParserParameters {
...
//		System.out.println(FSParameters.numAttributes);
	}
	...
	static boolean isReal(String paramName) {
		...
		if(paramName.equalsIgnoreCase("probMutation")) return true;
		if(paramName.equalsIgnoreCase("beta")) return true;
	...
	static boolean isInteger(String paramName) {
		...
		if(paramName.equalsIgnoreCase("numAttributes")) return true;
		return false;
	}
	...
	static void insertRealParameter(String paramName,String paramValue) {
		...
		try {
			FSParameters param=new FSParameters();
			java.lang.reflect.Field f= FSParameters.class.getField(paramName);
	...
	static String getLine() {
		...
		} while(st!=null && st.equalsIgnoreCase(""));//跳过空行
		return st;
	}
}
