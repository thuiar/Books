...
import GAssist.DiscretizationAlgorithms.Discretizer;

public class AdaptiveAttribute {
	public static void constructor(int []crm,int base,int attribute,InstanceWrapper ins) {
		if(Globals_ADI.types[attribute]==Attribute.REAL) {
			int disc=Rand.getInteger(0
				,DiscretizationManager.getNumDiscretizers()-1);
			int numMicroInt=DiscretizationManager.getDiscretizer(disc).getNumIntervals(attribute);
			...
			int i,pos,max;
			int sum=0;
			for(i=0,pos=base+2,max=numIntervals-1;i<max;i++,pos+=2) {
	...
	public static void myConstructor(int []crm,int base,int attribute,InstanceWrapper ins) {
		if(Globals_ADI.types[attribute]==Attribute.REAL) {
			int disc=Rand.getInteger(0
				,DiscretizationManager.getNumDiscretizers()-1);
			...
			int numIntervals;
			int maxAllowedIntervals=Math.min(numMicroInt,Parameters.maxIntervals);
			if(maxAllowedIntervals>2) 
				numIntervals=maxAllowedIntervals;				
			else 
				numIntervals = maxAllowedIntervals;
	...
	public static boolean doMatchReal(int []crm,int base,int value) {
		...
		for(int i=0,max=crm[base];i<max;i++) {
			if(value<crm[base2]) return (crm[base2+1]==1);
			base2+=2;
		}
		LogManager.printErr("Integrity error at AdaptiveAttribute.doMatchReal. "+crm[base]+" "+crm[base+1]+" "+value);
		System.exit(1);
		return false;
	}
	...
	public static String dumpPhenotype(int []crm,int base,int attribute) {
		...
		if(Globals_ADI.types[attribute]==Attribute.REAL) {
			double min=att.minAttribute();
			double max=att.maxAttribute();
	...
	public static int doSplit(int []crm,int base,int attribute,int interval) {
		...
		if(size<2 || crm[base]==Parameters.maxIntervals) return 0;
	...
	public static void doReinitialize(int []crm,int base,int attribute) {
		if(Globals_ADI.types[attribute]!=Attribute.REAL) return;
		constructor(crm,base,attribute,null);
		myConstructor(crm,base,attribute,null);
	}
}
