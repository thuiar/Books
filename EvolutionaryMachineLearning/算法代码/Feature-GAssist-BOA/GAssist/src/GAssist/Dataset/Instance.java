...
public class Instance {
	...
	public double distance(Instance is, int index){
		if(this.nominalValues[index]==is.nominalValues[index])
			return 0;
		else 
			return 1;
	}
	
	public double distance(Instance is){
		double sum=0;
		for(int i=0;i<is.nominalValues.length-1 ;i++){
			if(this.nominalValues[i]!=is.nominalValues[i])
				sum++;
		}
		return sum;
