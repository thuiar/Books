...
class TFN(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.post_fusion_layer_cat_1 = nn.Linear(self.text_out + self.video_hidden + self.audio_hidden, self.post_fusion_dim)
        self.post_fusion_layer_cat_2 = nn.Linear(self.post_fusion_dim, args.num_classes)
    ...
    def forward(self, text_x, audio_x, video_x):
        ...
        audio_h, _ , _ = self.audio_subnet(audio_x)
        video_h, _ , _ = self.video_subnet(video_x)
        ...
        #changed
        fusion_tensor_cat = torch.cat((audio_h, text_h, video_h), dim = 1)
        fusion_tensor_cat_1 = self.post_fusion_layer_cat_1(fusion_tensor_cat)
        output_cat = self.post_fusion_layer_cat_2(fusion_tensor_cat_1)

        res = {
            'Feature_t': text_h,
            'Feature_a': audio_h,
            'Feature_v': video_h,
            'Feature_f': fusion_tensor_cat,
            'Predicts': output_cat
        }
