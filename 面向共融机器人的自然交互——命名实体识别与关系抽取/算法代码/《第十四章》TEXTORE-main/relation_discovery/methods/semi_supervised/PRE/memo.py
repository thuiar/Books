...
class ODCMemory(object):
    ...
    def init_memory(self):
        kmeans = KMeans(self.num_classes, random_state=0)
        predict = kmeans.fit_predict(self.memo_smaple.numpy())
        self.pred = torch.Tensor(predict).long().cuda() # init pseudo label
        self.print_class()
        self.update_centroids_memory(cinds=[x for x in range(self.num_classes)])
    ...
    def _compute_centroids_by_weight(self):
        ...
        return centroids
    def weight_avg(self, i, x):
        c = self.memo_proto[i, :].unsqueeze(0)
        dist = torch.pow(c - x, 2).sum(1)
        weight = F.softmax(dist, dim=0).unsqueeze(1)
        return (weight * x).sum(0)

    def update_labeled_samples_memory(self, ind, feature):
    ...
    def update_samples_memory(self, ind, feature):
        ...
        self.memo_smaple[ind, ...] = feature_norm
        # compute new labels
        # ddd = self.memo_proto.unsqueeze(1) - feature_norm.unsqueeze(0)
        # similarity_to_centroids = - torch.pow(ddd, 2).sum(2)
    ...
    def update_memo_proto(self, c):
        self.memo_proto.copy_(c)
...
#         self.pred = None
#         self.logger = logger
#         self.kmeans = KMeans(n_clusters=num_classes, random_state=0)
#         self.memo_proto = torch.zeros((num_classes, hidden_size))
...
#         predict = self.kmeans.fit_predict(self.memo_smaple.numpy())
#         self.pred = torch.Tensor(predict).long().cuda() # pseudo label
#         self.print_class()
#     def shuffle_labels(self):
#         lab_lenght = self.pred.size(0)
#         inds = list(range(lab_lenght))
#         select_ind = np.random.choice(inds, size=int(lab_lenght * 0.2), replace=False)
#         select_label = self.pred[select_ind]
#         random.shuffle(select_label)
#         self.pred[select_ind] = select_label
...
#             kk = np.bincount(self.pred.cpu().numpy(), minlength=self.num_classes)
#             self.logger.info("class count: {}".format(kk))
#     def si_score(self, x=None, pred_y=None):
#         if x is None or pred_y is None:
#             x = self.memo_smaple.numpy()
#             pred_y = self.pred.cpu().numpy()
#         return silhouette_score(x, pred_y, sample_size=len(x), metric='euclidean')
