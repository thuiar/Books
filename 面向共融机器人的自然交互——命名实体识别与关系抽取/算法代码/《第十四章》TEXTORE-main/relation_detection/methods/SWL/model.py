...
class DTGNS(nn.Module):
    ...
    def forward(self, token, att_mask, pos, labels=None, train=False):
        ...
        if train:
            ...
            return out
    def detection_loss(self, x, lb, neg=None):
        loss = self.classifier(x, lb, neg)
        return loss
    def discovery_loss(self, x, lb, neg=None):
        loss = self.classifier.forward_fake_label(x, lb, neg)
        return loss
